$(function(){
    ...
    var socket = {
        ...
        onComeIn : function(){
            this.socket.on(this.app+'come in', function (user,online){
                $("#online").html(online);
                $("#onlineTotal").val(online);

                if($("#in"+user.id).length != 0) return;
                sysMes("<span id='in"+user.id+"'>欢迎 "+user.name+" 尊驾光临，有问题问美女，没问题聊美女</span>");
        ...
        onExit: function(){
            ...
            this.socket.on(this.app+'exit', function (user,online) {
                $("#online").html(online);
                $("#onlineBasis").val(online);
                if($("#out"+user.id).length != 0) return;
                sysMes("<span id='out"+user.id+"'> " + user.name + " 已退出聊天室</span>");
        ...
        emitComeIn : function(user){
            var onlineBasis = $("#onlineBasis").val();
            this.socket.emit(this.app+'come in',user,onlineBasis);
        },
    ...
    $(".yaoqing").on("click",function(){
        $("<div class='yaoqing-box'><div class='yaoqing-layer'></div><img src='/phone/images/share.png' class='yaoqing-layer-img'></div>").appendTo("body");
        $(".yaoqing-layer").css({
            width : $("body").width(),
            height : $("body").height()
        })
